...
def get_move_handler(row, col):
	...
	def handler(teleBot: TeleBot, query_id, chat_id, message_id, username):
		...
		if game.get_current_username() is not username:
			return

		msg_id = (chat_id, message_id)
		moved = game.make_move_current(row, col)
		if moved and game.game_over == 'None' and game.get_current_username() == '': # Jika pergerakan valid, lanjutkan dengan pergerakan bot
